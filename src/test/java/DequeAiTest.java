...
public class DequeAiTest {
    DequeAi<Integer> deque;
    ...
    @Test
    public void testRemoveSecondMultiItem() {
        deque.addBack(1);
        deque.addBack(2);
        deque.addBack(3);
        assertEquals("expect removeSecond to return the second item", Integer.valueOf(2), deque.removeSecond());
        assertEquals("expect size to decrement after removeSecond", 2, deque.size());
        assertEquals("expect removeSecond to preserve the other items", "1 3", show());
    }

    @Test
    public void testRemoveSecondTwoItemsRepairsBackLink() {
        deque.addBack(1);
        deque.addBack(2);
        assertEquals("expect removeSecond to return the last item when size is 2", Integer.valueOf(2), deque.removeSecond());
        deque.addBack(3);
        assertEquals("expect addBack to work after removeSecond removes the last item", "1 3", show());
        assertEquals("expect removeBack to use the repaired back link", Integer.valueOf(3), deque.removeBack());
        assertEquals("expect the front item to remain", "1", show());
    }

    @Test
    public void testRemoveSecondEmpty() {
        try {
            deque.removeSecond();
            fail("expect exception to be thrown for removeSecond from an empty list.");
        } catch (java.util.NoSuchElementException nsee) {
        }
    }
    ...
    public void testRemoveSecondSingleItem() {
        ...
        try {
            deque.removeSecond();
            fail("expect exception to be thrown for removeSecond from a single-item list.");
        } catch (java.util.NoSuchElementException nsee) {
        }
        assertEquals("expect size to remain unchanged after rejected removeSecond", 1, deque.size());
        assertEquals("expect item to remain after rejected removeSecond", "1", show());
    }
